...
export class BravoSplitComponent extends SplitComponent {
  ...
  private _color: string = "rgb(123, 169, 177)";
  ...
  public collapsePanel() {
    ...
    let _nSizeAutoWidth = (this.elRef.nativeElement.offsetWidth)
      - _nTotalSize - (this._gutterSize * (_areaSizes.length - 1));

    let _nSizeAutoHeight = (this.elRef.nativeElement.offsetHeight)
      - _nTotalSize - (this._gutterSize * (_areaSizes.length - 1));

    let _nIndexArea = (this._nGutterOrder + 1) / 2;
    let outputAreaSizes = this.getVisibleAreaSizes();
    let _nDefaultSize = +this.elRef.nativeElement.querySelectorAll('bravo-area')[_nIndexArea].getAttribute('size');

    if (this._direction === 'horizontal') {
      if (!isNullOrUndefined(_nIdxAreaAuto)) {
        _areaSizes[_nIdxAreaAuto] = _nSizeAutoWidth;
      }
    }
    else {
      if (!isNullOrUndefined(_nIdxAreaAuto)) {
        _areaSizes[_nIdxAreaAuto] = _nSizeAutoHeight;
      }
    }
    this.setNewAreaSize(outputAreaSizes, _nIndexArea, _areaSizes, _nIdxAreaAuto, _nDefaultSize);
    if(!isNullOrUndefined(_nIdxAreaAuto)) {
      outputAreaSizes[_nIdxAreaAuto] = "*";
    }
    this.setVisibleAreaSizes(outputAreaSizes);
  }

  private setNewAreaSize(_pOutputAreaSizes: IOutputAreaSizes, _pnIndexArea: number,
    _pAreaSizes: number[], _pnIdxAreaAuto: number, _pnDefaultSize: number) {
    if (this._zCollapseDir === 'top' || this._zCollapseDir === 'left') {
      if (isNaN(_pnDefaultSize)) {
        _pnDefaultSize = +this.elRef.nativeElement.querySelectorAll('bravo-area')[_pnIndexArea - 1].getAttribute('size');
        if (_pAreaSizes[_pnIndexArea - 1] !== _pnDefaultSize) {
          _pOutputAreaSizes[_pnIndexArea - 1] = _pnDefaultSize;
          return;
        }
      }

      if (_pAreaSizes[_pnIndexArea] !== _pnDefaultSize) {
        _pOutputAreaSizes[_pnIndexArea - 1] = _pAreaSizes[_pnIndexArea] - _pnDefaultSize + _pAreaSizes[_pnIndexArea - 1];
        _pOutputAreaSizes[_pnIndexArea] = _pnDefaultSize;
        return
      }
      _pOutputAreaSizes[_pnIndexArea] = _pAreaSizes[_pnIndexArea] + _pAreaSizes[_pnIndexArea - 1];
      _pOutputAreaSizes[_pnIndexArea - 1] = 0;
    }
    else {
      if (isNaN(_pnDefaultSize)) {;
        _pnDefaultSize = +this.elRef.nativeElement.querySelectorAll('bravo-area')[_pnIndexArea - 1].getAttribute('size');
      ...
      if (_pAreaSizes[_pnIndexArea] !== _pnDefaultSize) {
        _pOutputAreaSizes[_pnIndexArea - 1] = _pAreaSizes[_pnIndexArea] - _pnDefaultSize + _pAreaSizes[_pnIndexArea - 1];
        _pOutputAreaSizes[_pnIndexArea] = _pnDefaultSize;
        console.log(_pOutputAreaSizes)
        return;
      }

      _pOutputAreaSizes[_pnIndexArea] = 0;
      _pOutputAreaSizes[_pnIndexArea - 1] = _pAreaSizes[_pnIndexArea] + _pAreaSizes[_pnIndexArea - 1];
